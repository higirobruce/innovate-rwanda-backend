...
export default class CompanyTypes {
  ...
  static async addType(req, res) {
    try {
      ...
      return res
        .status(400)
        .send({ message: "Sorry, Failed to add company type at moment" });
    }
  }

  static async editType(req, res) {
    try {
      const update = await db["CompanyTypes"]
        .update((req.body), {
          where: {
            id: req.body.id
          },
  ...
  static async removeType(req, res) {
    try {
      const response = await db["CompanyTypes"]
        .destroy({
          where: {
            id: req.query.type
          },
      ...
      if (response) {
        ...
        return res.status(200).json({
          message: "Type not yet added"
        })
      }
    } catch (err) {
      return res
        .status(400)
        .send({ message: "Sorry, Failed to remove company type at moment" });
    }
  }
}